...
class SQLAlchemy:
    ...
    def __init__(
        ...
        *,
        ...
        """A SQLAlchemy declarative model class. Subclass this to define db
        models.
        ...
        db engine. Otherwise, it will use the default :attr:`metadata` and
    ...
    def init_app(self, app: Flask) -> None:
        ...
        must be called before accessing the db engine or session with the app.
        ...
        if not engine_options:
            raise RuntimeError(
                "Either 'SQLALCHEMY_db_URI' or 'SQLALCHEMY_BINDS' must be set."
            )
    ...
    def _apply_driver_defaults(self, options: dict[str, t.Any], app: Flask) -> None:
        ...
        SQLite in-memory dbs use ``StaticPool`` and disable ``check_same_thread``.
        ...
        if url.drivername in {"sqlite", "sqlite+pysqlite"}:
            if url.db is None or url.db in {"", ":memory:"}:
    ...
    def _call_for_binds(
        ...
        for key in keys:
            try:
                ...
                if key is None:
                    message = f"'SQLALCHEMY_db_URI' config is not set. {message}"
    ...
    def create_all(self, bind_key: str | None | list[str | None] = "__all__") -> None:
        """Create tables that do not exist in the db by calling
    ...
    def reflect(self, bind_key: str | None | list[str | None] = "__all__") -> None:
        """Load table definitions from the db by calling ``metadata.reflect()``
