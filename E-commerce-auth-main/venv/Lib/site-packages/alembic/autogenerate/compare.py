    ...
    def obj_removed(obj: _constraint_sig):
        if is_index_sig(obj):
            if obj.is_unique and not supports_unique_constraints:
                # many dbs double up unique constraints
...
def _correct_for_uq_duplicates_uix(
    ...
    # for reflection side, names are in their final db form
    # already since they're from the db
...
def _compare_type(
    ...
    if conn_type._type_affinity is sqltypes.NullType:
        log.info(
            "Couldn't determine db type " "for column '%s.%s'",
            tname,
            cname,
        )
        return
