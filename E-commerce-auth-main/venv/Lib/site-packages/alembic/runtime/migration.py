...
class MigrationContext:
    """Represent the db state made available to a migration
    script.

    :class:`.MigrationContext` is the front end to an actual
    db connection, or alternatively a string output
    stream given a particular db dialect,
    ...
    in the db, the :meth:`.MigrationContext.configure`
    method to create new :class:`.MigrationContext` objects.
    For example, to get at the current revision in the
    db using :meth:`.MigrationContext.get_current_revision`::
    ...
    def configure(
        ...
        :param url: a string db url, or a
    ...
    def autocommit_block(self) -> Iterator[None]:
        """Enter an "autocommit" block, for dbs that support AUTOCOMMIT
        isolation levels.

        This special directive is intended to support the occasional db
        DDL or system operation that specifically has to be run outside of
        any kind of transaction block.   The PostgreSQL db platform
        ...
        though the db overall supports transactional DDL.
        ...
        which must be run outside of a transaction block at the db level.
        ...
        to ensure that the db driver is not inside of a DBAPI level
        transaction block.

        .. warning::

            As is necessary, **the db transaction preceding the block is
    ...
    def begin_transaction(
        ...
        correspond to a real db transaction.   If the target db
        ...
        flag is not set, and the migration is against a real db
    ...
    def get_current_revision(self) -> Optional[str]:
        ...
        does not contain unmerged branches in the target db;
    ...
    def get_current_heads(self) -> Tuple[str, ...]:
        ...
        unmerged branches exist within the target db, the returned tuple
    ...
    def run_migrations(self, **kw: Any) -> None:
        ...
        against the db such as the ``current`` or ``stamp`` commands.
...
class MigrationInfo:
    ...
    it results in any actual db operations)."""
