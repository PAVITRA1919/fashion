...
class EnvironmentContext(util.ModuleClsProxy):
    ...
    ``with:`` statement.  A typical use of :class:`.EnvironmentContext`::
        ...
        def my_function(rev, context):
            ...
            will be the current db revision,
    ...
    itself as well as the current revision in the db.
    ...
    def is_transactional_ddl(self) -> bool:
        ...
        This defaults to the type of db in use, and
    ...
    def get_x_argument(
        ...
        For example, to support passing a db URL on the command line,
    ...
    def configure(
        ...
        **kw: Any,
        ...
        :class:`.EnvironmentContext` which will provide db
        connectivity and other configuration to a series of
        migration scripts.

        Many methods on :class:`.EnvironmentContext` require that
        this method has been called in order to function, as they
        ultimately need to have db access or at least access
        to the dialect in use.  Those which do are documented as such.

        The important thing needed by :meth:`.configure` is a
        means to determine what kind of db dialect is in use.
        An actual connection to that db is needed only if
        ...
        :param compare_server_default: Indicates server default comparison
         ...
         execute
         the two defaults on the db side to compare for equivalence.
        ...
        :param include_name: A callable function which is given
         the chance to return ``True`` or ``False`` for any db reflected
         object based on its name, including db schema names when
         ...
         * ``compare_to``: the object being compared against, if available,
         ...
         For the use case of omitting specific schemas from a target db
    ...
    def run_migrations(self, **kw: Any) -> None:
        ...
        db connection (if one is present).
        ...
        db in use, can be passed from a custom ``env.py`` script
