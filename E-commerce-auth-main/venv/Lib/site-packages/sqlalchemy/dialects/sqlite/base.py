...
db Locking Behavior / Concurrency
---------------------------------------

SQLite is not designed for a high level of write concurrency. The db
...
has exclusive access to the db during this period - all other
...
particular db file at any time; however, there are several
...
lock the db file once a transaction is started and DML (e.g. INSERT,
...
any SELECT statement.   This may lead to a SQLite db that locks
...
``BEGIN`` statement uses the "deferred" mode, where the db file is
...
The SQLite db supports transactional :term:`DDL` as well.
...
SQLite types are unlike those of most other db backends, in that
...
dbs in general, as the dot is a syntactically significant character,
...
class SQLiteExecutionContext(default.DefaultExecutionContext):
    ...
    def _translate_colname(self, colname):
        ...
        # "tablename.colname", or if using an attached db,
        # "db.tablename.colname", in cursor.description
...
class SQLiteDialect(default.DefaultDialect):
    ...
    def get_isolation_level(self, dbapi_connection):
        ...
        if res:
            ...
            # table level locking when db connections
    ...
    def _get_table_pragma(self, connection, pragma, table_name, schema=None):
        ...
        if schema is not None:
            ...
            # because PRAGMA looks in all attached dbs if no schema
