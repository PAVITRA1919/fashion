...
The file specification for the SQLite db is taken as the "db"
...
looks like::
    ...
    # relative path
    e = create_engine('sqlite:///path/to/db.db')
...
used. Double backslashes are probably needed::
    ...
    e = create_engine('sqlite:///C:\\path\\to\\db.db')

To use sqlite ``:memory:`` db specify it as the filename using
``sqlite:///:memory:``. It's also the default if no filepath is
present, specifying only ``sqlite://`` and nothing else::

    # in-memory db (note three slashes)
...
as the "db" portion of the SQLAlchemy url (that is, following a slash)::

    e = create_engine("sqlite:///file:path/to/db?mode=ro&uri=true")
...
Above, the pysqlite / sqlite3 DBAPI would be passed arguments as::
    ...
    sqlite3.connect(
        "file:path/to/db?mode=ro&nolock=1",
...
options for memory only dbs to be used in multiple threads.
...
``check_same_thread`` to ``False`` automatically whenever a file-based db
...
based on the kind of SQLite db that's requested:

* When a ``:memory:`` SQLite db is specified, the dialect by default
  will use :class:`.SingletonThreadPool`. This pool maintains a single
  connection per thread, so that all access to the engine within the current
  thread use the same ``:memory:`` db - other threads would access a
  different ``:memory:`` db.  The ``check_same_thread`` parameter
  defaults to ``True``.
* When a file-based db is specified, the dialect will use
  ...
  .. versionchanged:: 2.0
    ...
    SQLite file db engines now use :class:`.QueuePool` by default.
    Previously, :class:`.NullPool` were used.  The :class:`.NullPool` class
    may be used by specifying it via the
    :paramref:`_sa.create_engine.poolclass` parameter.

Disabling Connection Pooling for File dbs
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Pooling may be disabled for a file based db by specifying the
...
Using a Memory db in Multiple Threads
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

To use a ``:memory:`` db in a multithreaded scenario, the same
connection object must be shared among threads, since the db exists
...
Note that using a ``:memory:`` db in multiple threads requires a recent
version of SQLite.
...
temporary table in a file-based SQLite db across multiple checkouts
...
The SQLite db is weakly typed, and as such it is possible when using
binary values, which in Python are represented as ``b'some string'``, that a
particular SQLite db can have data values within different rows where
...
consistently, however if a particular SQLite db has data that was
...
an effort to minimize the SQLite dbs's file locking behavior, even
...
SQLAlchemy uses connection pooling with file-based SQLite dbs, so we need to ensure that the UDF is attached to the
...
class SQLiteDialect_pysqlite(SQLiteDialect):
    ...
    def _is_url_file_db(cls, url):
        if (url.db and url.db != ":memory:") and (
    ...
    def is_disconnect(self, e, connection, cursor):
        ...
        ) and "Cannot operate on a closed db." in str(e)
