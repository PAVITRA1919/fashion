...
def _format_url(url, driver, ident):
    ...
    if filename and filename != ":memory:":
        ...
        assert name_token, f"sqlite filename has no name token: {url.db}"
...
def _sqlite_post_configure_engine(url, engine, follower_ident):
    ...
    def connect(dbapi_connection, connection_record):
        ...
        dbapi_connection.execute(
            f'ATTACH db "{attach_path}" AS test_schema'
        )

    @event.listens_for(engine, "engine_disposed")
    def dispose(engine):
        """most dbs should be dropped using
...
def _sqlite_drop_db(cfg, eng, ident):
    _drop_dbs_w_ident(eng.url.db, eng.driver, ident)


def _drop_dbs_w_ident(dbname, driver, ident):
    for path in os.listdir("."):
        fname, ext = os.path.split(path)
        if ident in fname and ext in [".db", ".db.enc"]:
            log.info("deleting SQLite db file: %s", path)
            os.remove(path)
...
def _reap_sqlite_dbs(url, idents):
    ...
    for ident in idents:
        for drivername in _drivernames:
            _drop_dbs_w_ident(url.db, drivername, ident)
