...
in use as well as the network address of the db.   Assuming a datasource
...
db name to connect towards may be specified locally in the URL, rather
...
name must be URL encoded which means using plus signs for spaces::
    ...
    engine = create_engine("mssql+pyodbc://scott:tiger@myhost:port/dbname?driver=ODBC+Driver+17+for+SQL+Server")
...
<https://github.com/mkleehammer/pyodbc/wiki/Connecting-to-dbs>`_,
using the parameter ``odbc_connect``.  A :class:`_sa.engine.URL` object
can help make this easier::

    from sqlalchemy.engine import URL
    connection_string = "DRIVER={SQL Server Native Client 10.0};SERVER=dagger;db=test;UID=user;PWD=password"
...
Connecting to dbs with access tokens
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Some db servers are set up to only accept access tokens for login. For
example, SQL Server allows the use of Azure Active Directory tokens to connect
to dbs. This requires creating a credential object using the
...
db using Azure credentials::
    ...
    TOKEN_URL = "https://db.windows.net/"  # The token URL for any Azure SQL db

    connection_string = "mssql+pyodbc://@my-server.db.windows.net/myDb?driver=ODBC+Driver+17+for+SQL+Server"
...
will continue to maintain active db connections**, even when the
