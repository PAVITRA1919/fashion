...
def _mssql_create_db(cfg, eng, ident):
    with eng.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
        conn.exec_driver_sql("create db %s" % ident)
        conn.exec_driver_sql(
            "ALTER db %s SET ALLOW_SNAPSHOT_ISOLATION ON" % ident
        )
        conn.exec_driver_sql(
            "ALTER db %s SET READ_COMMITTED_SNAPSHOT ON" % ident
        )
...
def _reap_mssql_dbs(url, idents):
    ...
    with eng.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
        ...
        to_reap = conn.exec_driver_sql(
            "select d.name from sys.dbs as d where name "
        ...
        for total, dbname in enumerate(to_drop, 1):
            if _mssql_drop_ignore(conn, dbname):
                dropped += 1
        log.info(
            "Dropped %d out of %d stale dbs detected", dropped, total
        )
