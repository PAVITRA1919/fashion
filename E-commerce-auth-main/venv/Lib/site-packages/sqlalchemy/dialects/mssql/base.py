...
.. note::
    ...
    SQL Server db will instead reject the ``CREATE TABLE`` statement.
...
type deployed to the SQL Server db can be specified as ``Numeric`` using
...
.. tip::
    ...
    in most other SQL dbs. Users should explicitly set the
...
other words the db's configured default is used. This will
render::
...
* The flag begins with the value ``None``, before a db connection is
...
qualifier, that is, including the db name and owner name as separate
tokens, such as ``mydb.dbo.some_table``. These multipart names can be set
...
"db" and "owner"  components in order to correctly query the SQL
...
The above schema would be rendered as ``[Mydb].dbo``, and also in
reflection, would be reflected using "dbo" as the owner and "Mydb"
as the db name.

To control how the schema name is broken into db / owner,
specify brackets (which in SQL Server are quoting characters) in the name.
Below, the "owner" will be considered as ``Mydb.dbo`` and the
"db" will be None::
...
To individually specify both db and owner name with special characters
or embedded dots, use two sets of brackets::

    Table(
        "some_table", metadata,
        Column("q", String(50)),
        schema="[Mydb.Period].[MyOwner.Dot]"
    )


.. versionchanged:: 1.2 the SQL Server dialect now treats brackets as
   identifier delimiters splitting the schema into separate db
...
db level. This allows, for instance, to run a db that
is compatible with SQL2000 while running on a SQL2005 db
server. ``server_version_info`` will always return the db
server version information (in this case SQL2005) and not the
compatibility level information. Because of this, if running under
a backwards compatibility mode SQLAlchemy may attempt to use T-SQL
statements that are unable to be parsed by the db server.
...
Enabling snapshot isolation for the db as a whole is recommended
for modern levels of concurrency support.  This is accomplished via the
following ALTER db commands executed at the SQL prompt::

    ALTER db Mydb SET ALLOW_SNAPSHOT_ISOLATION ON

    ALTER db Mydb SET READ_COMMITTED_SNAPSHOT ON
    ...
    def bind_processor(self, dialect):
        def process(value):
            if isinstance(value, datetime.datetime):
                ...
                """issue #5339
                per: https://github.com/mkleehammer/pyodbc/wiki/Tips-and-Tricks-by-db-Platform#time-columns
...
class ROWVERSION(TIMESTAMP):
    ...
    db as itself; the returned datatype will be
...
def _schema_elements(schema):
    ...
    #           OwnerPlusDBTest.test_owner_db_pairs
        ...
        s = (
            ...
            .outerjoin(
                ...
                onclause=sql.and_(
                    ...
                    == columns.c.column_name.collate("db_DEFAULT"),
                ),
            )
            .outerjoin(
                identity_cols,
