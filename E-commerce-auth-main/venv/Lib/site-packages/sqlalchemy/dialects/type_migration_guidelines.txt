...
1. the TypeEngine classes are used for:
    ...
    b. Specifying types that are entirely specific to the db
    ...
    but the db in use takes vendor-specific flags for those
    types.
...
indicate a special type only available in this db, it must be *removed* from the 
module and from this dictionary.

6. "ischema_names" indicates string descriptions of types as returned from the db
...
a subclass of compiler.GenericTypeCompiler.
    ...
    all cases, regardless of whether or not that type is legal on the backend db.
