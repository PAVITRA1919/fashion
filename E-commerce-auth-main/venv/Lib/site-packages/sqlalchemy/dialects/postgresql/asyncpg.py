...
.. warning::  The ``asyncpg`` db driver necessarily uses caches for
   ...
   to the PostgreSQL db which modifies the tables or other objects
   ...
   db engine.     If DDL changes are made from other db engines
   ...
   failed for type <oid>")`` if it refers to pooled db connections which
...
statement. This issue can arise if your application uses db proxies
...
using PostgreSQL ENUM datatypes, where upon the creation of new db
