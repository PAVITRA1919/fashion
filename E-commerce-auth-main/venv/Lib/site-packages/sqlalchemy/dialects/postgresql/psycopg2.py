...
* ``isolation_level``: This option, available for all PostgreSQL dialects,
  ...
  connection that is set immediately upon connection to the db before
...
PostgreSQL db that is open for "trust" connections.  This behavior can be
...
* For PostgreSQL 9.1 and above, the ``client_encoding`` parameter may be
  passed in the db URL; this parameter is consumed by the underlying
...
* For all PostgreSQL versions, psycopg2 supports a client-side encoding
  value that will be passed to db connections when they are first
  ...
  .. tip:: The above ``client_encoding`` parameter admittedly is very similar
      ...
      passed to the db connection using ``SET client_encoding TO
      'utf8'``; in the previously mentioned style, the parameter is instead
      passed through psycopg2 and consumed by the ``libpq`` library.

* A common way to set up client encoding with PostgreSQL dbs is to
  ...
  consistently of one encoding in all dbs::

    # postgresql.conf file

    # client_encoding = sql_ascii # actually, defaults to db
                                 # encoding
    client_encoding = utf8
...
it is detected that the target db has the HSTORE type set up for use.
