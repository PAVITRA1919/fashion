...
.. note::
   ...
   Will generate on the backing db as::
...
handles and statement caches.   The PostgreSQL db includes a variety
...
to **determine the default schema for the current db connection**.
...
However, if your db username **matches the name of a schema**, PostgreSQL's
...
establishes basic db cross-compatibility for :meth:`.Operators.match`
with other backends.
...
different than the actual name of the column as expressed in the db.
...
When using CONCURRENTLY, the PostgreSQL db requires that the statement
...
The PostgreSQL db creates a UNIQUE INDEX implicitly whenever the
...
class PGInspector(reflection.Inspector):
    ...
    def get_table_oid(
        ...
        :param schema: string schema name; if omitted, uses the default schema
         of the db connection.  For special quoting,
         use :class:`.quoted_name`.
    ...
    def has_type(
        ...
        """Return if the db has the specified type in the provided
        schema.
