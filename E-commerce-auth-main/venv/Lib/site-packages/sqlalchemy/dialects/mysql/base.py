...
Within the realm of SQLAlchemy, the two dbs have a small number of
syntactical and behavioral differences that SQLAlchemy accommodates automatically.
To connect to a MariaDB db, no changes to the db URL are required::
...
backing db reports as MariaDB.  Based on this flag, the dialect
...
and is not compatible with a MySQL db.    To use this mode of operation,
...
detection detects that the backing db is not MariaDB.
...
"mariadb" prefix will be present in the option names when the db URL
...
.. seealso::
    ...
    with timed out connections as well as db restarts.
...
that is created in a MySQL / MariaDB db with a storage engine
...
foreign key declarations are *always* received from the db
...
db itself, especially if db reflection features are
to be used.
...
the db connection will return true for the value of
``SELECT @@autocommit;``.
...
db that includes codepoints more than three bytes in size, this new
charset is preferred, if supported by both the db as well as the client
DBAPI, as in::
...
emit a warning when attempting to pass binary data to the db, while a
...
existing db structures.  The detection is entirely automatic and
...
However, to accommodate the vast majority of MySQL dbs that do not
...
accommodate newer dbs that specify ``explicit_defaults_for_timestamp``,
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _get_server_version_info(self, connection):
        # get db server version info explicitly over the wire
    ...
    def _get_default_schema_name(self, connection):
        return connection.exec_driver_sql("SELECT db()").scalar()
    ...
    def has_table(self, connection, table_name, schema=None, **kw):
        ...
        try:
            ...
            # 1049: Unknown db '%s'  - for nonexistent schema
            #
            # also added:
    ...
    def _correct_for_mysql_bugs_88718_96365(self, fkeys, connection):
        ...
        if schema_by_table_by_column:
            ...
            # that was used in CREATE db and CREATE TABLE, but
