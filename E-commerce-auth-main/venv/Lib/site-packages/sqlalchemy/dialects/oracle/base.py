...
examples which assume the usage of an autoincrement-capable db.   To
...
The Oracle db supports "READ COMMITTED" and "SERIALIZABLE" modes of
...
.. note:: The implementation for the
   ...
   its first connection to the db in order to acquire the
   ...
   db-related failure, the level is assumed to be "READ COMMITTED".  No
   warning is emitted for this initial first-connect condition as it is
   expected to be a common restriction on Oracle dbs.
...
actual server version in order to assist with migration of Oracle dbs,
...
db migration that wishes to "DROP CONSTRAINT" on a name that was
...
of the generation of truncated names, and to fully review and test all db
...
The Oracle db supports RETURNING fully for INSERT, UPDATE and DELETE
...
present as a "marker" for this type; additionally, when a db column
...
check for such limitations, only the db will.
...
class OracleCompiler(compiler.SQLCompiler):
    ...
    statements to work under non-ANSI configured Oracle dbs, if
    ...
    def _get_nonansi_join_whereclause(self, froms):
        ...
        def visit_join(join):
            if join.isouter:
                # https://docs.oracle.com/db/121/SQLRF/queries006.htm#SQLRF52354
