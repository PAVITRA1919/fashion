...
cx_Oracle provides several methods of indicating the target db.  The
dialect translates from a series of different URL forms.

Hostname Connections with Easy Connect Syntax
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Given a hostname, port and service name of the target Oracle db, for
...
Alternatively, if no port, db name, or ``service_name`` is provided, the
...
The cx_Oracle dialect connects to this db service when ``myalias`` is the
hostname portion of the URL, without specifying a port, db name or
``service_name``::
...
wallet as shown in cx_Oracle documentation `Connecting to Autononmous dbs
...
"db name" portion of the URL as below::
...
Using Oracle db Resident Connection Pooling (DRCP)
--------------------------------------------------------
...
<https://cx-oracle.readthedocs.io/en/latest/user_guide/connection_handling.html#db-resident-connection-pooling-drcp>`_.
...
pooling and Oracle db additionally uses DRCP::
...
Oracle related software is via the `NLS_LANG <https://www.oracle.com/db/technologies/faq-nls-lang.html>`_
...
the Oracle db is configured with a Unicode-aware character set, as well
...
In the case that the Oracle db is not configured with a Unicode character
...
For the unusual case that data in the Oracle db is present with a broken
...
https://cx-oracle.readthedocs.io/en/latest/api_manual/module.html#db-types.
...
class OracleDialect_cx_oracle(OracleDialect):
    ...
    def create_connect_args(self, url):
        ...
        if db or service_name:
            # if we have a db, then we have a remote host
            port = url.port
            if port:
                port = int(port)
            else:
            ...
            if db and service_name:
                raise exc.InvalidRequestError(
                    ...
                    'be used with a "db" part of the url'
                )
            if db:
                makedsn_kwargs = {"sid": db}
            if service_name:
    ...
    def is_disconnect(self, e, connection, cursor):
        (error,) = e.args
        if isinstance(
            e, (self.dbapi.InterfaceError, self.dbapi.dbError)
        ...
        if re.match(r"^(?:DPI-1010|DPI-1080|DPY-1001|DPY-4011)", str(e)):
            ...
            # python-oracledb's DPY-1001: not connected to db
            # python-oracledb's DPY-4011: the db or network closed the
            # connection
            # TODO: others?
            return True
