...
"""private module containing functions used to convert db
...
def load_on_ident(
    ...
    *,
    ...
    """Load the given identity key from the db."""
    if key is not None:
        ident = key[1]
...
def load_on_pk_identity(
    ...
    *,
    ...
    """Load the given primary key identity from the db."""
    ...
    if require_pk_cols and only_load_props:
        ...
        if has_changes:
            ...
            # to the db before tinkering like that (session.refresh() is
            # tinkering).
            raise sa_exc.InvalidRequestError(
...
def _validate_version_id(mapper, state, dict_, row, getter):
    if mapper._get_state_attr_by_column(
        ...
        raise orm_exc.StaleDataError(
            ...
            "does not match db-loaded version id '%s'."
            % (
                state_str(state),
