...
def create_engine(url: Union[str, _url.URL], **kwargs: Any) -> Engine:
    ...
    The standard calling form is to send the :ref:`URL <db_urls>` as the
    first positional argument, usually a string
    that indicates db dialect and connection arguments::

        engine = create_engine("postgresql+psycopg2://scott:tiger@localhost/test")

    .. note::

        Please review :ref:`db_urls` for general guidelines in composing
    ...
    ``dialect`` is a db name such as ``mysql``, ``oracle``,
    ...
    :param creator: a callable which returns a DBAPI connection.
        ...
        connection pool and will be used to create all new db
        connections. Usage of this function causes connection
        parameters specified in the URL argument to be bypassed.

        This hook is not as flexible as the newer
        :meth:`_events.DialectEvents.do_connect` hook which allows complete
        control over how a connection is made to the db, given the full
    ...
    :param max_identifier_length: integer; override the max_identifier_length
        ...
        is the db's configured maximum number of characters that may be
        used in a SQL identifier such as a table name, column name, or label
        name. All dialects determine this value automatically, however in the
        case of a new db version for which this value has changed but
...
def engine_from_config(
    ...
    which provides the :ref:`db URL <db_urls>`.
