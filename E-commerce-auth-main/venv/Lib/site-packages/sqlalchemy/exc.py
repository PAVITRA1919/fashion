...
class NoSuchModuleError(ArgumentError):
    """Raised when a dynamically-loaded module (usually a db dialect)
...
class InvalidatePoolError(DisconnectionError):
    ...
    entire pool should be invalidated, as the db has been restarted.
...
class NoResultFound(InvalidRequestError):
    """A db result was required but none was found.
...
class MultipleResultsFound(InvalidRequestError):
    """A single db result was required but more than one were found.
...
class UnboundExecutionError(InvalidRequestError):
    """SQL was attempted without a db connection to execute it on."""
...
class DBAPIError(StatementError):
    """Raised when the execution of a db operation fails.

    Wraps exceptions raised by the DB-API underlying the
    db operation.  Driver-specific implementations of the standard
...
class NotSupportedError(dbError):
