...
which automatically generates mapped classes and relationships from a db
...
model from an existing db on the fly.  By addressing the issue strictly
...
.. tip:: The :ref:`automap_toplevel` extension is geared towards a
   ...
   and pre-named relationships can be generated on the fly from a db
...
The simplest usage is to reflect an existing db into a new model.
...
explicit table declaration::
    ...
    # we can reflect it ourselves from a db, using options
...
In the example above, if the db contained a table named ``accounts`` in
...
the information reflected about a db column before the :class:`_schema.Column`
...
class AutomapBase:
    ...
    def prepare(
        ...
        :param schema: Schema name to reflect when reflecting tables using
         ...
         in use by the db connection is used.
