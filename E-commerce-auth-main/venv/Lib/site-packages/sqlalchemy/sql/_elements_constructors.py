...
def between(
    ...
    :param symmetric: if True, will render " BETWEEN SYMMETRIC ". Note
     that not all dbs support this syntax.
...
def bindparam(
    ...
    Above, we see that ``Wendy`` is passed as a parameter to the db,
    while the placeholder ``:name_1`` is rendered in the appropriate form
    for the target db, in this case the PostgreSQL db.
    ...
    :param type\_:
      ...
      passed to the db.  For example, a :func:`.bindparam` which
    ...
    :param literal_execute:
      ...
      capability is for rendering LIMIT / OFFSET clauses for db
...
def case(
    *whens: Union[
    ...
    :param else\_: An optional SQL expression which will be the evaluated
      ...
      dbs will produce a result of NULL if none of the "when"
...
def column(
    ...
    like the name of a db column; if the string contains mixed case,
...
def extract(field: str, expr: _ColumnExpressionArgument[Any]) -> Extract:
    ...
    db where the ``YEAR`` component matches a specific value.
    ...
    The implementation of ``EXTRACT`` may vary across db backends.
    Users are reminded to consult their db documentation.
    """
...
def funcfilter(
    ...
    for db backends that support the "FILTER" clause.
...
def over(
    ...
    for db backends that support window functions.
