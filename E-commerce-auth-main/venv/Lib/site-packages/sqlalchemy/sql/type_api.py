...
class TypeEngine(Visitable, Generic[_T]):
    ...
    class Comparator(
        ...
        def _adapt_expression(
            ...
            "days delta" behavior by the db (in reality, most dbs
    ...
    def column_expression(
        ...
        columns to be wrapped in some special db function in order
    ...
    def bind_expression(
        ...
        that require literals being wrapped in some special db function
        in order to coerce an application-level value into a db-specific
...
class Emulated(TypeEngineMixin):
    ...
    An :class:`.Emulated` type will use an available db type
    in conjunction with Python-side routines and/or db constraints
    in order to approximate the behavior of a db type that is provided
...
class TypeDecorator(SchemaEventTarget, ExternalType, TypeEngine[_T]):
    ...
    by dbs as adding a number of days to the given date.
    ...
    def process_result_value(
        ...
        being received in result rows coming from the db.
        This method is called at **result fetching time** and is passed
        the literal Python data value that's extracted from a db result
        row.
...
def adapt_type(
    ...
    for t in typeobj.__class__.__mro__[0:-1]:
        ...
        return typeobj
    # if we adapted the given generic type to a db-specific type,
