...
"""The schema module provides the building blocks for db metadata.

Each element within this module describes a db entity which can be
...
schema elements, and can also be associated with an actual db connection
such that operations involving the contained elements can contact the db
as needed.
...
class SchemaItem(SchemaEventTarget, visitors.Visitable):
    """Base class for items that define a db schema."""
...
class Table(
    ...
    .. seealso::
        ...
        :ref:`metadata_describing` - Introduction to db metadata
    ...
    def __init__(
        ...
        **kw: Any,
        ...
        :param name: The name of this table as represented in the db.
        ...
        :param extend_existing: When ``True``, indicates that if this
            ...
            operation against the db, even if a :class:`_schema.Table`
            ...
            objects
            both reflected from the db, as well as the given
        ...
        :param schema: The schema name for this table, which is required if
            ...
            for the engine's db connection.  Defaults to ``None``.
        ...
        # load column definitions from the db if 'autoload' is defined
    ...
    def _sentinel_column_characteristics(
        ...
        as an INSERT..RETURNING sentinel for a particular db
        dialect.
    ...
    def append_column(
        ...
        as it exists within any underlying db, assuming that
        table has already been created in the db.   Relational
        dbs support the addition of columns to existing tables
    ...
    def append_constraint(self, constraint: Union[Index, Constraint]) -> None:
        ...
        relational db automatically, for a table that already exists
        in the db.   To add a constraint to an
        existing relational db table, the SQL ALTER command must
...
class Column(DialectKWArgs, SchemaItem, ColumnClause[_T]):
    ...
    def __init__(
        ...
        **dialect_kwargs: Any,
        ...
        :param name: The name of this column as represented in the db.
          ...
          behavior applies even for dbs which standardize upper
        ...
        :param autoincrement: Set up "auto increment" semantics for an
          ...
          * **DDL rendering** (i.e. :meth:`.MetaData.create_all`,
            ...
            will imply that db-specific keywords such as PostgreSQL
            ``SERIAL``, MySQL ``AUTO_INCREMENT``, or ``IDENTITY`` on SQL Server
            should also be rendered.  Not every db backend has an
            ...
            be produced in the db.

          * **INSERT semantics** (i.e. when a :func:`_sql.insert` construct is
            compiled into a SQL string and is then executed on a db using
            ...
            ORM when ORM-mapped objects are persisted to the db,
          ...
          * DDL issued for the column, if the column does not already include
            ...
            :class:`.Identity`, will include db-specific
            ...
            * SQLite - SQLite integer primary key columns are implicitly
              ...
              by the db vendor.  See the section
          ...
          * When a single-row :func:`_sql.insert` construct is compiled and
            ...
            using a method specific to the db driver in use:
        ...
        :param default: A scalar, Python callable, or
            ...
            *default value* for this column, which will be invoked upon insert
            ...
            which creates a default generator on the db side.
        ...
        :param nullable: When set to ``False``, will cause the "NOT NULL"
            ...
            .. note::
                ...
                PostgreSQL db allows nullable identity column by
        ...
        :param server_default: A :class:`.FetchedValue` instance, str, Unicode
            ...
            valid SQLAlchemy expressions or constructs::
                ...
                engine = create_engine(
                    'postgresql+psycopg2://scott:tiger@localhost/mydb'
                )
            ...
            column will generate a default value on the db side which
            will be available to SQLAlchemy for post-fetch after inserts. This
            construct does not specify any DDL and the implementation is left
            to the db, such as via a trigger.
        ...
        :param server_onupdate: A :class:`.FetchedValue` instance
            representing a db-side default generation function,
            such as a trigger. This
            indicates to SQLAlchemy that a newly generated value will be
            available after updates. This construct does not actually
            implement any kind of generation function within the db,
        ...
        :param unique: When ``True``, and the :paramref:`_schema.Column.index`
            ...
            a db-specific naming convention to take place.
        ...
        :param system: When ``True``, indicates this is a "system" column,
             ...
             db, and should not be included in the columns list for a
...
def insert_sentinel(
    ...
    *,
    ...
    corresponding db table actually has this column present, so if adding
    it to an existing model, existing db tables would need to be migrated
...
class ForeignKey(DialectKWArgs, SchemaItem):
    ...
    def __init__(
        ...
        **dialect_kw: Any,
        ...
        :param name: Optional string. An in-db name for the key if
    ...
    def _column_tokens(self) -> Tuple[Optional[str], str, Optional[str]]:
        ...
        # name. Some dbs (e.g. Sybase) support
        # inter-db foreign keys. See tickets#1341 and --
...
class IdentityOptions:
    """Defines options for a named db sequence or an identity column.
...
class Sequence(HasSchemaAttr, IdentityOptions, DefaultGenerator):
    ...
    def __init__(
        ...
        :param start: the starting index of the sequence.  This value is
         used when the CREATE SEQUENCE command is emitted to the db
        ...
        :param increment: the increment value of the sequence.  This
         ...
         the db as the value of the "INCREMENT BY" clause.  If ``None``,
         the clause is omitted, which on most platforms indicates an
         increment of 1.
        :param minvalue: the minimum value of the sequence.  This
         value is used when the CREATE SEQUENCE command is emitted to
         the db as the value of the "MINVALUE" clause.  If ``None``,
        ...
        :param maxvalue: the maximum value of the sequence.  This
         value is used when the CREATE SEQUENCE command is emitted to
         the db as the value of the "MAXVALUE" clause.  If ``None``,
         the clause is omitted, which on most platforms indicates a
        ...
        :param nominvalue: no minimum value of the sequence.  This
         value is used when the CREATE SEQUENCE command is emitted to
         the db as the value of the "NO MINVALUE" clause.  If ``None``,
         the clause is omitted, which on most platforms indicates a
         minvalue of 1 and -2^63-1 for ascending and descending sequences,
         respectively.
        ...
        :param nomaxvalue: no maximum value of the sequence.  This
         value is used when the CREATE SEQUENCE command is emitted to
         the db as the value of the "NO MAXVALUE" clause.  If ``None``,
         the clause is omitted, which on most platforms indicates a
         maxvalue of 2^63-1 and -1 for ascending and descending sequences,
         respectively.
        ...
        :param cycle: allows the sequence to wrap around when the maxvalue
         ...
         is emitted to the db as the "CYCLE" clause.  If the limit is
    ...
    def create(self, bind: _CreateDropBind, checkfirst: bool = True) -> None:
        """Creates this sequence in the db."""

        bind._run_ddl_visitor(ddl.SchemaGenerator, self, checkfirst=checkfirst)

    def drop(self, bind: _CreateDropBind, checkfirst: bool = True) -> None:
        """Drops this sequence from the db."""
...
class FetchedValue(SchemaEventTarget):
    """A marker for a transparent db-side default.

    Use :class:`.FetchedValue` when the db is configured
...
class Constraint(DialectKWArgs, HasConditionalDDL, SchemaItem):
    ...
    def __init__(
        ...
        **dialect_kw: Any,
        ...
        :param name:
          Optional, the in-db name of this ``Constraint``.
...
class CheckConstraint(ColumnCollectionConstraint):
    ...
    def __init__(
        ...
        **dialect_kw: Any,
        ...
        :param name:
          Optional, the in-db name of the constraint.
...
class ForeignKeyConstraint(ColumnCollectionConstraint):
    ...
    def __init__(
        ...
        **dialect_kw: Any,
        ...
        :param name: Optional, the in-db name of the key.
...
class MetaData(HasSchemaAttr):
    ...
    def reflect(
        ...
        **dialect_kwargs: Any,
        ...
        r"""Load all available table definitions from the db.

        Automatically creates ``Table`` entries in this ``MetaData`` for any
        table available in the db but not yet present in the
        ``MetaData``.  May be called multiple times to pick up tables recently
        added to the db, however no special action is taken if a table
        in this ``MetaData`` no longer exists in the db.
    ...
    def __init__(
        ...
        :param persisted:
          ...
          * ``True``, will render ``GENERATED ALWAYS AS ... STORED``, or the
            equivalent for the target db if supported.
          * ``False``, will render ``GENERATED ALWAYS AS ... VIRTUAL``, or
            the equivalent for the target db if supported.

          Specifying ``True`` or ``False`` may raise an error when the DDL
          is emitted to the target db if the db does not support
          that persistence option.   Leaving this parameter at its default
          of ``None`` is guaranteed to succeed for all dbs that support
          ``GENERATED ALWAYS AS``.
