...
class String(Concatenable, TypeEngine[str]):
    ...
    def __init__(
        ...
        :param length: optional, a length for the column for use in
          ...
          TABLE`` will be issued.  Certain dbs may require a
          ``length`` for use in DDL, and will raise an exception when
          the ``CREATE TABLE`` DDL is issued if a ``VARCHAR``
          with no length is included.  Whether the value is
          interpreted as bytes or characters is db specific.
        ...
        :param collation: Optional, a column-level collation for
          ...
          .. note::
            ...
            correct types are used on the db.
...
class Text(String):
    ...
    do not have a length; while some dbs will accept a length
...
class Unicode(String):
    ...
    transmit and receive data to the db is usually determined by the
    DBAPI itself. All modern DBAPIs accommodate non-ASCII strings but may have
    different methods of managing db encodings; if necessary, this
    ...
    .. warning:: Some db backends, particularly SQL Server with pyodbc,
...
class Numeric(HasExpressionLookup, TypeEngine[_N]):
    ...
    .. note::
        ...
        When using a :class:`.Numeric` datatype against a db type that
    ...
    def __init__(
        ...
        :param decimal_return_scale: Default scale to use when converting
         ...
         db types don't have a notion of "scale", so by default the
    ...
    def __init__(
        ...
        :param precision: the numeric precision for use in DDL ``CREATE
           ...
           .. note:: For the Oracle backend, the
              ...
              To create a db agnostic :class:`_types.Float` that
...
class DateTime(
    ...
    def __init__(self, timezone: bool = False):
         ...
         **base date/time-holding type only**.   It is recommended
         ...
         using this flag, as some dbs include separate generic
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
    ...
    from the db in a result set, the string value is always checked
    against the list of possible values and a ``LookupError`` is raised
    if no match is found.  When passing a value to the db as a
    ...
    are persisted to the db; the values of the Python Enum, here
    ...
    def __init__(self, *enums: object, **kw: Any):
        ...
        :param metadata: Associate this type directly with a ``MetaData``
           object. For types that exist on the target db as an
        ...
        :param name: The name of this type. This is required for PostgreSQL
           and any future supported db which requires an explicitly
        ...
        :param native_enum: Use the db's native ENUM type when
        ...
        :param schema: Schema name of this type. For types that exist on the
           target db as an independent schema construct (PostgreSQL),
        ...
        :param validate_strings: when True, string values that are being
           passed to the db in a SQL statement will be checked
        ...
        :param values_callable: A callable which will be passed the PEP-435
           ...
           using the string value of an enum to be persisted to the db
        ...
        :param sort_key_function: a Python callable which may be used as the
           ...
           default, the db value of the enumeration is used as the
           sorting function.
...
class Interval(Emulated, _AbstractInterval, TypeDecorator[dt.timedelta]):
    ...
    def __init__(
        ...
        :param native: when True, use the actual
          INTERVAL type provided by the db, if
...
class JSON(Indexable, TypeEngine[Any]):
    ...
    **Customizing the JSON Serializer**
    ...
    will be re-fetched from the db within the context of retrieving
    generated defaults.
...
class ARRAY(
    ...
    * When emitting the type declaration itself to the db, e.g.
      ``INTEGER[][]``

    * When translating Python values to db values, and vice versa, e.g.
    ...
    * When used with the Python ``getitem`` accessor, the number of dimensions
    ...
    def __init__(
        ...
        :param dimensions: if non-None, the ARRAY will assume a fixed
         ...
         on the db, how it goes about interpreting Python and
...
class TIMESTAMP(DateTime):
    ...
    def __init__(self, timezone: bool = False):
        ...
        :param timezone: boolean.  Indicates that the TIMESTAMP type should
         enable timezone support, if available on the target db.
         On a per-dialect basis is similar to "TIMESTAMP WITH TIMEZONE".
         If the target db does not support timezones, this flag is
         ignored.
...
class Uuid(Emulated, TypeEngine[_UUID_RETURN]):
    """Represent a db agnostic UUID datatype.
...
class UUID(Uuid[_UUID_RETURN], type_api.NativeForEmulated):
    ...
    This is the SQL-native form of the :class:`_types.Uuid` db agnostic
    datatype, and is backwards compatible with the previous PostgreSQL-only
    version of ``UUID``.

    The :class:`_sqltypes.UUID` datatype only works on dbs that have a
