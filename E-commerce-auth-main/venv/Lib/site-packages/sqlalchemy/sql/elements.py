...
class ColumnElement(
    ...
    def _non_anon_label(self) -> Optional[str]:
        ...
        db-specific basis; e.g. an expression like ``MAX(table.col)`` may
        appear as the string ``max`` on one db (like PostgreSQL) or may
...
class BinaryExpression(OperatorExpression[_T]):
    ...
    """Indicates that any db will know this is a boolean expression
    even if the db does not have an explicit boolean datatype.
...
class WithinGroup(ColumnElement[_T]):
    ...
    It's supported only by certain db backends, such as PostgreSQL,
...
class FunctionFilter(Generative, ColumnElement[_T]):
    ...
    It's supported only by certain db backends.
...
class quoted_name(util.MemoizedSlots, str):
    ...
    "name normalize" option.  Certain db backends, such as
