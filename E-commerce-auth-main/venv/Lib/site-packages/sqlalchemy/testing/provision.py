...
def create_follower_db(follower_ident):
    for cfg in _configs_for_db_operation():
        log.info("CREATE db %s, URI %r", follower_ident, cfg.db.url)
...
def drop_follower_db(follower_ident):
    for cfg in _configs_for_db_operation():
        log.info("DROP db %s, URI %r", follower_ident, cfg.db.url)
...
def _configs_for_db_operation():
    ...
    for cfg in config.Config.all_configs():
        url = cfg.db.url
        backend = url.get_backend_name()
        host_conf = (backend, url.username, url.host, url.db)
...
def create_db(cfg, eng, ident):
    """Dynamically create a db for testing.
...
def drop_db(cfg, eng, ident):
    """Drop a db that we dynamically created for testing."""
...
def update_db_opts(db_url, db_opts, options):
    """Set db options (db_opts) for a test db that we created."""
...
def follower_url_from_main(url, ident):
    """Create a connection URL for a dynamically-created test db.
    ...
    return url.set(db=ident)


@register.init
def configure_follower(cfg, ident):
    """Create dialect-specific config settings for a follower db."""
    pass


@register.init
def run_reap_dbs(url, ident):
    """Remove dbs that were created during the test process, after the
    process has ended.

    This is an optional step that is invoked for certain backends that do not
    reliably release locks on the db as long as a process is still in
