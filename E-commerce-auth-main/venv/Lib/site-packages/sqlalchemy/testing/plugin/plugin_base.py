...
def setup_options(make_option):
    ...
    make_option(
        ...
        type=str,
        dest="db",
        help="Use prefab db uri. Multiple OK, "
    ...
    make_option(
        ...
        type=str,
        dest="dburi",
        help="db uri.  Multiple OK, first one is run by default.",
    )
    make_option(
        "--dbdriver",
        action="append",
        type=str,
        dest="dbdriver",
        help="Additional db drivers to include in tests.  "
        "These are linked to the existing db URLs by the "
        "provisioning system.",
    )
    make_option(
        "--dropfirst",
        action="store_true",
        dest="dropfirst",
        help="Drop all tables in the target db first",
    )
    make_option(
        "--disable-asyncio",
        action="store_true",
    ...
    make_option(
        ...
        type=str,
        help="name of template db to use for PostgreSQL "
        "CREATE db (defaults to current db)",
    )
    make_option(
        "--oracledb-thick-mode",
        action="store_true",
...
def _engine_uri(options, file_config):
    ...
    for db_url in expanded_urls:
        log.info("Adding db URL: %s", db_url)
...
@post
def _prep_testing_db(options, file_config):
    from sqlalchemy.testing import config
